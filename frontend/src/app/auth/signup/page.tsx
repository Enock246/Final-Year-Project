...
import { CheckCircle2, ChevronRight, Loader2, Eye, EyeOff, ShieldCheck, AlertCircle, ArrowLeft } from 'lucide-react';
...
export default function SignupPage() {
  ...
  const handleNextStep = async () => {
    const isStep1Valid = await trigger(['fullName', 'phone', 'studentId']);
    if (isStep1Valid) {
      setStep(2);
    }
  };
  ...
  if (verificationSent) {
    return (
      <main className="flex-1 flex flex-col items-center justify-center p-4 md:p-6 bg-canvas-soft min-h-screen">
        <motion.div
          initial={{ opacity: 0, y: 10, scale: 0.95 }}
          animate={{ opacity: 1, y: 0, scale: 1 }}
          transition={{ duration: 0.4, ease: [0.25, 1, 0.5, 1] }}
          className="w-full max-w-md bg-canvas p-6 md:p-8 rounded-xl shadow-none border border-hairline space-y-6 flex flex-col justify-center items-center text-center overflow-hidden"
        >
          ...
          <form onSubmit={handleVerifyOtp} className="w-full space-y-4">
            <div className="space-y-1.5 w-full">
              <input
                ...
                placeholder="000000"
                className="w-full h-12 px-4 text-center tracking-[1em] text-2xl font-light rounded-2xl border border-black/10 bg-canvas text-ink focus:outline-none focus:border-primary-press focus:ring-[3px] focus:ring-primary-press/10 transition-all"
  ...
  if (isSubmitting) {
    return (
      <main className="flex-1 flex flex-col items-center justify-center p-4 md:p-6 bg-canvas-soft min-h-screen">
        <div className="w-full bg-canvas p-6 md:p-8 rounded-xl shadow-none border border-hairline max-w-md space-y-6">
  ...
  return (
    <main className="flex-1 flex flex-col p-3 md:p-6 bg-canvas-soft min-h-screen items-center justify-center">
      <div className="w-full max-w-[480px] mx-auto">
        ...
        >
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 md:space-y-5 relative overflow-hidden">
              ...
              {step === 1 && (
                ...
                >
                  <div>
                    ...
                    {errors.fullName && <p className="text-ruby text-[13px] mt-1.5 leading-tight">{errors.fullName.message}</p>}
                  </div>
                  ...
                  <div>
                    ...
                    {errors.phone && <p className="text-ruby text-[13px] mt-1.5 leading-tight">{errors.phone.message}</p>}
                  </div>
                  ...
                  <div>
                    ...
                    {errors.studentId && <p className="text-ruby text-[13px] mt-1.5 leading-tight">{errors.studentId.message}</p>}
                  </div>
                  ...
                  <div className="mt-auto pt-6 z-10">
                    <button
                      ...
                      className="w-full bg-primary text-white text-[16px] font-medium min-h-[48px] px-4 rounded-pill hover:bg-primary-press transition-colors flex items-center justify-center gap-2 shadow-sm"
                    >
                      Continue
              ...
              {step === 2 && (
                ...
                >
                  <div>
                    ...
                    {errors.email && <p className="text-ruby text-[13px] mt-1.5 leading-tight">{errors.email.message}</p>}
                  </div>
                  ...
                  <div>
                    ...
                    {errors.password && <p className="text-ruby text-[13px] mt-1.5 leading-tight">{errors.password.message}</p>}
                  </div>
                    ...
                    <label className="flex items-start gap-3 cursor-pointer group">
                      <div className="relative flex items-center justify-center mt-0.5 shrink-0">
                        <input 
                          {...register('terms')} 
                          type="checkbox" 
                          className="peer w-[18px] h-[18px] appearance-none rounded-sm border border-black/10 bg-canvas checked:bg-primary checked:border-primary transition-all cursor-pointer" 
                        />
